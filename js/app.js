...
/**
 * 獲取新聞詳情（從緩存讀取，若沒有則從後端按 slug 查詢）
 */
async function getArticleBySlugAsync(slug) {
  // 先從本地緩存查找
  let source = _newsCache || NEWS_DATA;
  let article = source.find(n => n.slug === slug);
  if (article) return article;

  // 緩存中沒有，嘗試從後端按 slug 查詢
  if (API_BASE !== undefined) {
    try {
      ...
      console.warn('後端查詢文章失敗:', e);
    }
  }
  return null;
}

/** 保留向後兼容的同步查找（找不到時返回 null） */
...
async function renderArticle(slug) {
  ...
  app.innerHTML = `
    ...
    <div class="container" style="padding:4rem 1rem;text-align:center;">
      <p style="color:var(--color-gray-500);">載入中...</p>
    </div>
  `;
    ...
    <article class="article-page">
      <div class="container article-container">
        ...
        <div class="article-footer">
          ...
          ${(() => {
            const u = (typeof getWechatUser === 'function') ? getWechatUser() : null;
            return (u && u.isAdmin) ? `
              <button class="btn-primary-outline" onclick="navigateToEdit('${article.id}')">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
async function submitPublishArticle(e) {
  ...
  try {
    ...
    if (data.success) {
      ...
      showToast('新聞發布成功！');
      window.location.hash = 'article/' + encodeURIComponent(data.article.slug);
    } else {
...
/**
 * 提交新聞編輯（發送 PUT 到後端 API）
 */
async function submitEditArticle(e, articleId) {
  e.preventDefault();
  const title = document.getElementById('pub-title').value.trim();
  const category = document.getElementById('pub-category').value;
  const excerpt = document.getElementById('pub-excerpt').value.trim();
...
async function getArticleById(id) {
  ...
  const source = _newsCache || NEWS_DATA;
  let article = source.find(n => n.id === numericId);
  if (article) return article;
  // 從 API 獲取所有文章
  try {
    const res = await fetch(`${API_BASE}/api/news?pageSize=200`);
    ...
    if (data.articles) {
      _newsCache = data.articles;
      return data.articles.find(n => n.id === numericId) || null;
    }
  } catch (e) {
    console.warn('獲取文章失敗:', e);
  }
  return null;
}
...
async function renderEdit(id) {
    ...
    app.innerHTML = `
      ...
      <div class="publish-page">
        <div class="container publish-container">
          <div class="publish-card" style="text-align:center;padding:4rem 2rem;">
            <h1 class="publish-title">權限不足</h1>
            <p style="margin:1rem 0 2rem;color:var(--color-gray-600);">編輯新聞需要管理員登入。</p>
            <button class="btn-primary" onclick="requireLoginThenPublish()">管理員登入</button>
          </div>
        </div>
      </div>
  ...
  app.innerHTML = `
    ...
    <div class="publish-page">
      <div class="container publish-container">
        <div class="publish-card" style="text-align:center;padding:4rem 2rem;">
          <p style="color:var(--color-gray-500);">載入文章資料...</p>
        </div>
      </div>
    </div>
  `;
  ...
  if (!article) {
    app.innerHTML = `
      ...
      <div class="publish-page">
        <div class="container publish-container">
          <div class="publish-card" style="text-align:center;padding:4rem 2rem;">
            <h1 class="publish-title">文章不存在</h1>
            <p style="margin:1rem 0 2rem;color:var(--color-gray-600);">找不到指定的文章。</p>
            <button class="btn-primary-outline" onclick="goHome()">返回首頁</button>
          </div>
        </div>
      </div>
  ...
  if (article.coverImageUrl) {
    window.publishCoverImage = article.coverImageUrl;
  } else {
    window.publishCoverImage = '';
  }
  window.publishImageLink = '';
  ...
  const editForm = `
    <div class="publish-card">
      <h1 class="publish-title">編輯新聞</h1>
      <p class="publish-subtitle">修改文章內容後點擊「更新文章」。</p>
      ...
      <form onsubmit="submitEditArticle(event, ${article.id})">
        <div class="form-group">
          <label class="form-label" for="pub-title">標題 *</label>
          <input type="text" id="pub-title" class="form-input" placeholder="輸入新聞標題" value="${escapeHtml(article.title)}" required />
        </div>
        <div class="form-group">
          <label class="form-label" for="pub-category">分類 *</label>
          <select id="pub-category" class="form-select" required>
            ${CATEGORIES.filter(c => c.slug !== 'all').map(c => `<option value="${c.name}" ${c.name === article.category ? 'selected' : ''}>${c.name}</option>`).join('')}
          </select>
        </div>
        <div class="form-group">
          <label class="form-label" for="pub-excerpt">摘要</label>
          <textarea id="pub-excerpt" class="form-textarea" rows="3" placeholder="簡短描述新聞內容（選填）">${escapeHtml(article.excerpt || '')}</textarea>
        </div>
        <div class="form-group">
          <label class="form-label">封面圖片（直接上傳）</label>
          ${article.coverImageUrl ? `
            <div id="cover-preview-box" style="display:block;">
              <div class="upload-preview"><img id="cover-preview-img" src="${article.coverImageUrl}" alt="封面預覽" /></div>
              <div class="upload-link-box">
                <input type="text" id="cover-link-input" class="upload-link-input" value="${article.coverImageUrl}" readonly />
                <button type="button" class="btn-primary" onclick="copyImageLink('cover-link-input')">複製連結</button>
              </div>
            </div>
          ` : ''}
          <div class="upload-zone" onclick="document.getElementById('pub-cover-file').click()">
            ...
            <p>點擊選擇新的封面圖片（可選）</p>
            <p class="form-hint">上傳新圖片會替換現有封面</p>
          </div>
          ${!article.coverImageUrl ? '<div id="cover-preview-box" style="display:none;"></div>' : ''}
        </div>
        <div class="form-group">
          <label class="form-label" for="pub-content">正文 *</label>
          <textarea id="pub-content" class="form-textarea" rows="12" placeholder="輸入新聞正文..." required>${escapeHtml(plainContent)}</textarea>
          <p class="form-hint">可用空行分隔段落</p>
        </div>
        <div class="form-actions">
          <button type="button" class="btn-primary-outline" onclick="goHome()">取消</button>
          <button type="submit" class="btn-primary">更新文章</button>
        </div>
      </form>
    </div>
  `;
  ...
  app.innerHTML = `
    ...
    <footer class="site-footer">
      <div class="container">
        <p>© 2026 澳門最新聞。版權所有。</p>
        <p class="footer-sub">優雅精緻的新聞發布與管理系統</p>
        <p class="footer-deploy">部署於阿里雲 · 支援電郵登入與圖片上傳</p>
      </div>
    </footer>
  `;
